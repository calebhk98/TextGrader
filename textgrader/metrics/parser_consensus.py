...
default even when the suite itself is on)
    ...
    download. ``constituency_benepar`` was verified installable
    (``pip install benepar``) but constructing ``benepar.Parser('benepar_en3')``
    in this environment raises
    ``AttributeError: 'T5Tokenizer' object has no attribute
    'build_inputs_with_special_tokens'`` against the ``transformers`` version
    several other suites in this codebase need (see
    ``requirements-embeddings.txt``); that failure is reported verbatim as
    this feature's "unavailable" reason rather than silently skipped, and
    downgrading ``transformers`` to fix it is out of scope here because it
    would break ``coherence_suite``/``logic_suite``'s own model channels.
