...
def _shape_finding(metric_id: str, name: str, values: Sequence[float], unit: str, *,
                   ...
                   aggregation: str = "headline is the median of one best-match value per "
                                      "unit that had at least one candidate",
    ...
    dist["aggregation"] = aggregation
    return finding(metric_id, name, summary.get(statistic), unit, family=FAMILY,
...
def _sentence_pipeline(sentences: Sequence[str],
    ...
    if _feature(config, "simhash"):
        ...
        out.append(_shape_finding(
            ...
            # Whole-number bit distances: the median was 26 or 28 on five
            # different novels, so the mean carries the resolution.
            statistic="mean",
            aggregation="headline is the mean nearest-neighbor Hamming distance, one value per "
                        "unit that had at least one candidate; median and shape in distribution"))
    else:
