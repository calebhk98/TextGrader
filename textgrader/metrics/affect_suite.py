...
def _enabled(features: Mapping[str, Any], spec: EngineSpec) -> bool:
    if spec.kind == "vad":
        # One switch for every VAD engine, overridable per engine by name.
        return bool(features.get(spec.name, features.get("vad", spec.default_enabled)))
...
#: The two VAD resources textgrader.lexicons loads, kept as separate engines
#: because they are independent norm sets on different scales: NRC VAD 2.1
#: rates every dimension on [-1, 1] (0 is neutral), Warriner et al. on a 1-9
#: scale.  Only NRC valence is ``signed``: its zero means neutral, so its
#: positive/negative shares, reversals and cross-engine sign disagreement
#: mean something.  Warriner's midpoint is 5, and arousal and dominance are
#: intensities rather than polarities, so those five get the level,
#: volatility, arc, gap, spread and correlation findings only.
VAD_RESOURCES = {
    "nrc_vad": ("NRC VAD", "[-1, 1]", {"valence"}),
    "warriner_vad": ("Warriner VAD", "1-9", set()),
}


def _vad_scorer_factory(resource: str, dimension: str):
    def make_scorer(settings: Mapping[str, Any]):
        from textgrader import lexicons, text as textlib

        table, reason, _metadata = lexicons.norm_table(resource)
        if table is None:
            return None, reason

        def scorer(sentence: str) -> float | None:
...
def _vad_version(resource: str) -> Callable[[], str]:
    def version() -> str:
        from textgrader import lexicons

        return str(getattr(lexicons.resource_info(resource), "version", None) or "unknown")
    return version


def _register_vad_engines() -> None:
    for resource, (label, scale, signed_dims) in VAD_RESOURCES.items():
        for dimension in VAD_DIMENSIONS:
            register_engine(EngineSpec(
                name=f"{resource}_{dimension}",
                label=f"{label} {dimension} (mean over the sentence's covered words)",
                unit=f"{dimension} {scale}", signed=dimension in signed_dims, kind="vad",
                resource=f"textgrader.lexicons:{resource}", version=_vad_version(resource),
                make_scorer=_vad_scorer_factory(resource, dimension)), replace=True)
...
def _vad_placeholder_findings() -> list[dict[str, Any]]:
    ...
    for dimension in VAD_DIMENSIONS:
        ...
        if any(spec.kind == "vad" and name.endswith(f"_{dimension}")
