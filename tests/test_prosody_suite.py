...
def test_line_and_stanza_structure_headline_values():
    ...
    assert findings["rhythm.prosody_words_per_line"]["value"] == pytest.approx(8.5)
...
def test_meter_findings_report_feet_and_dispersion():
    findings = _measure(SONNET)
    assert findings["rhythm.prosody_feet_per_line"]["value"] == pytest.approx(5.0)
    assert findings["rhythm.prosody_syllables_per_line"]["value"] == pytest.approx(10.0)
