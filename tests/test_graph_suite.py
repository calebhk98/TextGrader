...
def test_the_booknlp_shim_holds_only_while_booknlp_loads():
    """``load_state_dict`` is shared by every torch model in the process, so
    the patch must be gone once BookNLP has built its models: fastcoref,
    benepar or an NLI model loaded afterwards must see the original."""

    torch = pytest.importorskip("torch")
    original = torch.nn.Module.load_state_dict
    with optional.shim_booknlp_transformers():
        assert torch.nn.Module.load_state_dict is not original
        # The one legacy key is dropped; every other key still loads strictly.
        layer = torch.nn.Linear(2, 2)
        state = dict(layer.state_dict())
        state["embeddings.position_ids"] = torch.zeros(1)
        layer.load_state_dict(state)
        with pytest.raises(RuntimeError):
            layer.load_state_dict({**layer.state_dict(), "unexpected.weight": torch.zeros(1)})
