...
def test_vad_placeholder_until_registered_then_wired_automatically(monkeypatch):
    # Start from a registry without the lexicon-backed VAD engines, so this
    # tests the registration path itself.
    monkeypatch.setattr(affect, "_ENGINES",
                        {name: spec for name, spec in affect._ENGINES.items()
                         if spec.kind != "vad"})
...
def _toy_vad_tables(monkeypatch):
    from textgrader import lexicons

    tables = {
        "nrc_vad": {"joy": {"valence": 0.9, "arousal": 0.5, "dominance": 0.4},
                    "grief": {"valence": -0.8, "arousal": -0.2, "dominance": -0.5}},
        "warriner_vad": {"joy": {"valence": 8.2, "arousal": 5.0, "dominance": 6.0},
                         "grief": {"valence": 1.9, "arousal": 4.0, "dominance": 3.0}},
    }
    monkeypatch.setattr(lexicons, "norm_table",
                        lambda name, path=None: (tables[name], None, {"name": name}))
    monkeypatch.setattr(affect, "_SCORER_CACHE", {})


def test_the_lexicon_vad_engines_score_each_sentence_by_its_covered_words(monkeypatch):
    _toy_vad_tables(monkeypatch)
    # Two covered words average; an uncovered word does not count as zero.
    assert affect.score_text("nrc_vad_valence", "Joy and grief, unnamed.")[0] == pytest.approx(0.05)
    assert affect.score_text("warriner_vad_valence", "Joy! Joy.")[0] == pytest.approx(8.2)
    assert affect.score_text("nrc_vad_arousal", "Nothing covered here.")[0] is None
    # NRC valence is centred on zero, so it gets sign-based findings;
    # Warriner's 1-9 scale and every arousal/dominance engine do not.
    assert affect._ENGINES["nrc_vad_valence"].signed
    assert not affect._ENGINES["warriner_vad_valence"].signed
    assert not affect._ENGINES["nrc_vad_arousal"].signed


def test_one_vad_switch_turns_every_vad_engine_off(monkeypatch):
    _toy_vad_tables(monkeypatch)
    analysis = _analysis("Joy came first. Then grief came. Joy returned at last.")
    on = _by_id(affect.measure(analysis, {"features": {"vad": True}}))
    assert on[f"{PREFIX}nrc_vad_valence_level"]["value"] == pytest.approx((0.9 - 0.8 + 0.9) / 3)
    assert f"{PREFIX}vad_valence_level" not in on  # the placeholder gives way
    off = _by_id(affect.measure(analysis, {"features": {"vad": False}}))
    assert not any("_vad_" in metric_id for metric_id in off)
